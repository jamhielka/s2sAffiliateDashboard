<template>
  ...
  >
   ...
   <a @click="handleClick(slotData)">{{ slotData.item.amount }}</a>
    ...
    <template v-slot:top>
      <v-toolbar flat>
        <v-toolbar-title>Cash-Out Request</v-toolbar-title>
...
<script>
import CORequestDialog from "../dialogs/CORequest.vue";
export default {
  components: {
 CORequestDialog,
  },
  data: () => ({
    CORequestDialog: { dialog: false },
    ...
    headers: [
      ...
      { text: "Account No.", value: "acctNumber" },
  ...
  methods: {
    ...
    async getForMerchantList() {
      ...
      await this.$api
        .post("/Admin/AdminCashOutRequest", this.GetRequest, {
          //   headers: {
